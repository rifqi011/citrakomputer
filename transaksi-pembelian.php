...
<?php
$tgl = date('Y-m-d');
$pembelian = mysqli_query($conn, "SELECT * FROM invoice_pembelian WHERE invoice_date = '$tgl'");
$jmlPembelian = mysqli_num_rows($pembelian);
$jmlPembelian1 = $jmlPembelian + 1;
$nomorInvoice = str_pad($jmlPembelian1, 3, "0", STR_PAD_LEFT);

$today = date("Ymd");
$in = "Pembelian-" . $today . $nomorInvoice;
?>
            ...
            <form role="form" id="form-tambah-invoice" method="post" action="transaksi-pembelian-input-no-invoice.php">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" readonly value="<?= $in ?>" id="invoice_pembelian_number_input" name="invoice_pembelian_number_input" required="">
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#example1 tbody').on('click', '.tblInsert', function() {
            ...
            window.location.href = "transaksi-pembelian-add?id=" + data0;
        });
